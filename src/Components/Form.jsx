...
function Form(props) {
  ...
  return (
    <form onSubmit={handleSubmit} className='flex flex-col shadow-lg p-3 gap-3 bg-slate-400 h-80 w-64 sm:w-80 items-center justify-center rounded-lg '>
        ...
        <input required onChange={(e)=>setEmail(e.target.value)} value={email} className=' border-none text-[14px] sm:text-[15px] outline-none p-2 rounded-lg w-52 sm:w-60' placeholder='Enter email..' />
        ...
        <input required onChange={(e)=>setPassword(e.target.value)} value={password} className=' border-none text-[14px] sm:text-[15px] outline-none p-2 rounded-lg w-52 sm:w-60' placeholder='Enter password' />
